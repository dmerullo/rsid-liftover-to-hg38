...
if __name__ == '__main__':
    ...
    for ln in myopen('$PATH_TO/SNPHistory.bcp.gz'):
	fd = ln.strip().split('\t')
	if ln.find('re-activ') < 0:
	    RS_HISTORY.add(fd[0])

    # record rs number merge history
    for ln in myopen('$PATH_TO/RsMergeArch.bcp.gz'):
	fd = ln.strip().split('\t')
	h, l = fd[0], fd[1]
	c = fd[6]
	# print 'c=', c
	...
	while True:
		...
		if rsCurrent not in RS_HISTORY:
		    print "rs" + rs + '\t', "lifted\t" + "rs" + rsCurrent
		    break
		else:
		    rs = rsLow
	    else:
		print "rs" + rs + '\t', "unlifted\t" + "rs" + rs
		break
